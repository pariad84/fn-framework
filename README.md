...
- **Builder** -- drag components (`text`, `box`, `popup`, `button`,
  ...
  components inside a `box` or a `popup`'s content area freely. "Save
...
- `text` and `box` were the first two components. `box` needing to accept
  drops itself -- so a `text`/`box` can nest inside another `box` -- led to
...
- `popup` needed the same drop-target behavior `box` has, but into an inner
  content div rather than its own header. Generalized `box`/`popup` around
  an `el.content` convention instead of hardcoding `'box'` in the save/
